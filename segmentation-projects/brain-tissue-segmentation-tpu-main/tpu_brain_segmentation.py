# -*- coding: utf-8 -*-
"""tpu_brain_segmentation.ipynb

This data was obtained from MR images ,then annotated and augmented.
Binary masks contain only brain tissue, excluding the tumor tissue.
...
X = np.load('/content/drive/MyDrive/Colab Notebooks/X_train_beyin.npy')
y = np.load('/content/drive/MyDrive/Colab Notebooks/Y_train_beyin.npy')
...
def conv_block(filter_size, kernel_size, padding = 'same', kernel_init = 'he_normal', activation = 'relu'):
  ...
  def make(inputs):
    ...
    #features = Activation(activation)(features)
...
with strategy.scope():
  ...
  model.compile(optimizer = tf.keras.optimizers.Adam(learning_rate = 1e-5) ,  loss = 'binary_crossentropy' , metrics = [dice_coef])
  hist = model.fit(X_train, y_train, validation_split=0.25  , batch_size = 16, epochs = 40 )
